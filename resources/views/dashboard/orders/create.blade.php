    ...
    <form action="{{ route('dashboard.orders.store') }}" method="post" enctype="multipart/form-data">
        ...
        <div class="row">
            ...
            <x-adminlte-input name="transaction_reference" label="{{ __('order.transaction_reference') }}"
                value="{{ old('transaction_reference') }}" type="text" placeholder="ex: TxRef.******"
            ...
            {{-- Shipping Fee --}}
            <x-adminlte-input name="shipping_fee" label="{{ __('order.shipping_fee') }}" value="{{ old('shipping_fee') }}"
                type="text" placeholder="ex: 10.10" oninput="this.value=this.value.replace(/[^0-9.]/g,'');"
                fgroup-class="col-md-6" />


            {{-- Books Total --}}
            <x-adminlte-input name="books_total" label="{{ __('order.books_total') }}" value="{{ old('books_total') }}"
                type="text" placeholder="ex: 10.10" oninput="this.value=this.value.replace(/[^0-9.]/g,'');"
                fgroup-class="col-md-6" />

            {{-- Total --}}
            <x-adminlte-input name="total" label="{{ __('order.total') }}" value="{{ old('total') }}" type="text"
                placeholder="ex: 10.10" oninput="this.value=this.value.replace(/[^0-9.]/g,'');" fgroup-class="col-md-6" />

            {{-- Status --}}
            <x-adminlte-select name="status" label="{{ __('order.status') }}" fgroup-class="col-md-6">
                <option value="" selected>-- {{ __('actions.choose') }} --</option>
                <option @if (old('status') == 'delivered') selected @endif value="delivered">{{ __('order.delivered') }}
                </option>
                <option @if (old('status') == 'pending') selected @endif value="pending">{{ __('order.pending') }}
                </option>
                <option @if (old('status') == 'refunded') selected @endif value="refunded">{{ __('order.refunded') }}
                </option>
            </x-adminlte-select>
            ...
            <x-adminlte-select name="payment_status" label="{{ __('order.payment_status') }}" fgroup-class="col-md-6">
                <option value="" selected>-- {{ __('actions.choose') }} --</option>
                <option @if (old('payment_status') == 'paid') selected @endif value="paid">{{ __('order.paid') }}</option>
                <option @if (old('payment_status') == 'pending') selected @endif value="pending">{{ __('order.pending') }}
                </option>
                <option @if (old('payment_status') == 'refunded') selected @endif value="refunded">{{ __('order.refunded') }}
                </option>
            </x-adminlte-select>
            ...
            <x-adminlte-select name="payment_type" label="{{ __('order.payment_type') }}" fgroup-class="col-md-6">
                <option value="" selected>-- {{ __('actions.choose') }} --</option>
                <option @if (old('payment_type') == 'cash') selected @endif value="cash">{{ __('order.cash') }}</option>
                <option @if (old('payment_type') == 'visa') selected @endif value="visa">{{ __('order.visa') }}</option>
            </x-adminlte-select>

            {{-- Tax Amount --}}
            <x-adminlte-input name="tax_amount" label="{{ __('order.tax_amount') }}" value="{{ old('tax_amount') }}"
                type="text" placeholder="ex: 10.10" oninput="this.value=this.value.replace(/[^0-9.]/g,'');"
                fgroup-class="col-md-6" />

            {{-- Address --}}
            <x-adminlte-input name="address" label="{{ __('order.address') }}" value="{{ old('address') }}" type="text"
            ...
            <x-adminlte-select name="shipping_area_id" label="{{ __('order.shipping_area_id') }}" fgroup-class="col-md-6">
                ...
                @if (!empty($getData['shipping_areas']) && isset($getData['shipping_areas']))
                    @foreach ($getData['shipping_areas'] as $shipping)
                        <option @if (old('shipping_area_id') == $shipping->id) selected="selected" @endif value="{{ $shipping->id }}">
            ...
            <x-adminlte-select name="user_id" label="{{ __('order.user_id') }}" fgroup-class="col-md-6">
                ...
                @if (!empty($getData['users']) && isset($getData['users']))
                    @foreach ($getData['users'] as $user)
                        <option @if (old('user_id') == $user->id) selected="selected" @endif value="{{ $user->id }}">
                            {{ $user->name }}</option>
                    @endforeach
                @else
                    <div class="alert alert-secondary" role="alert">
