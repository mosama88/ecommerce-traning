        ...
        <div class="col-12 col-sm-12">
            <div class="card card-info">
                <div class="card-header">
                    <h3 class="card-title"> {{ __('order.order_edit') }}</h3>
                </div>
                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <x-adminlte-input disabled name="number" label="{{ __('order.number') }}"
                                value="{{ $order['number'] }}" type="text" placeholder="ex: 10.10"
                            ...
                            <x-adminlte-input name="shipping_fee" label="{{ __('order.shipping_fee') }}"
                                value="{{ old('shipping_fee', $order['shipping_fee']) }}" type="text"
                            ...
                            <x-adminlte-input name="books_total" label="{{ __('order.books_total') }}"
                                value="{{ old('books_total', $order['books_total']) }}" type="text"
                                placeholder="ex: 10.10" oninput="this.value=this.value.replace(/[^0-9.]/g,'');"
                                fgroup-class="col-md-6" />

                            {{-- Total --}}
                            <x-adminlte-input name="total" label="{{ __('order.total') }}"
                            ...
                            <x-adminlte-select name="status" label="{{ __('order.status') }}" fgroup-class="col-md-6">
                                ...
                                <option @if (old('status', $order['status']) == 'delivered') selected @endif value="delivered">
                                    {{ __('order.delivered') }}
                                </option>
                                <option @if (old('status', $order['status']) == 'pending') selected @endif value="pending">
                                    {{ __('order.pending') }}
                                </option>
                                <option @if (old('status', $order['status']) == 'refunded') selected @endif value="refunded">
                                    {{ __('order.refunded') }}
                                </option>
                            </x-adminlte-select>

                            {{-- Payment Status --}}
                            <x-adminlte-select name="payment_status" label="{{ __('order.payment_status') }}"
                                fgroup-class="col-md-6">
                                <option value="" selected>-- {{ __('actions.choose') }} --</option>
                                <option @if (old('payment_status', $order['payment_status']) == 'paid') selected @endif value="paid">
                                    {{ __('order.paid') }}</option>
                                <option @if (old('payment_status', $order['payment_status']) == 'pending') selected @endif value="pending">
                                    {{ __('order.pending') }}
                                </option>
                                <option @if (old('payment_status', $order['payment_status']) == 'refunded') selected @endif value="refunded">
                                    {{ __('order.refunded') }}
                                </option>
                            </x-adminlte-select>

                            {{-- Payment Type --}}
                            <x-adminlte-select name="payment_type" label="{{ __('order.payment_type') }}"
                                fgroup-class="col-md-6">
                                <option value="" selected>-- {{ __('actions.choose') }} --</option>
                                <option @if (old('payment_type', $order['payment_type']) == 'cash') selected @endif value="cash">
                                    {{ __('order.cash') }}</option>
                                <option @if (old('payment_type', $order['payment_type']) == 'visa') selected @endif value="visa">
                                    {{ __('order.visa') }}</option>
                            </x-adminlte-select>

                            {{-- Tax Amount --}}
                            <x-adminlte-input name="tax_amount" label="{{ __('order.tax_amount') }}"
                                value="{{ old('tax_amount', $order['tax_amount']) }}" type="text"
                                placeholder="ex: 10.10" oninput="this.value=this.value.replace(/[^0-9.]/g,'');"
                                fgroup-class="col-md-6" />
                            ...
                            {{-- Shipping Area  --}}
                            <x-adminlte-select name="shipping_area_id" label="{{ __('order.shipping_area_id') }}"
                                fgroup-class="col-md-6">
                                <option value="" selected>-- {{ __('actions.choose') }} --</option>
                                @if (!empty($getData['shipping_areas']) && isset($getData['shipping_areas']))
                                    @foreach ($getData['shipping_areas'] as $shipping)
                                        <option @if (old('shipping_area_id', $order['shipping_area_id']) == $shipping->id) selected="selected" @endif
                                            value="{{ $shipping->id }}">
...
@push('js')
    <script>
        ...
        const checkCodeUrl = "{{ route('orders.check.code') }}";
