        ...
        <div class="col-12 col-sm-12">
            <div class="card card-primary">
                <div class="card-header">
                    <h3 class="card-title"> {{ __('books.books_edit') }}</h3>
                </div>

                <form action="{{ route('dashboard.books.update', $publisher->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <input type="hidden" name="books_id" value="{{ $publisher->id }}">
